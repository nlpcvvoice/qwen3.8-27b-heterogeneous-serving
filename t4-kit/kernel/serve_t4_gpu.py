...
# The GPU image keeps libcuda.so.1 OUTSIDE default ldconfig paths
# (/usr/local/nvidia/lib64). cmake (CUDA::cuda_driver) and the llama.cpp CUDA
# backend both need it — scan for it, set LD_LIBRARY_PATH, symlink into dirs
# cmake searches.
...
def publish(phase, **extra):
    ...
    if not CFG["ntfy_topic"]:
        return
    try:
        body = {"topic": CFG["ntfy_topic"], "title": f"t4-kit {phase}",
...
def start_control_listener():
    """Remote kill switch: watch_job/stop_job posts CONTROL/stop to our ntfy topic;
...
def self_test():
    ...
    # 4 simultaneous requests — concurrent-readiness proof
    n = 4
    errs = []
    t0 = time.time()
    def one():
